...
class DashboardV2Repository(BaseRepository[Website]):
    ...
    def seo_issue_rows(self, *, start: datetime, end: datetime) -> list[dict[str, Any]]:
        """Return SEO issue rows used by deterministic recommendations."""

        statement = (
            select(
                CrawlSession.website_id,
                SeoAnalysisIssue.id,
                SeoAnalysisIssue.severity,
                SeoAnalysisIssue.code,
                SeoAnalysisIssue.message,
                SeoAnalysisIssue.created_at,
            )
            .join(SeoAnalysis, SeoAnalysis.id == SeoAnalysisIssue.seo_analysis_id)
            .join(CrawlSession, CrawlSession.id == SeoAnalysis.crawl_session_id)
            .where(
                func.lower(SeoAnalysisIssue.severity).in_(("critical", "major")),
                SeoAnalysisIssue.created_at >= start,
                SeoAnalysisIssue.created_at <= end,
            )
            .order_by(SeoAnalysisIssue.created_at.desc())
            .limit(100)
        )
        return [
            {
    ...
    def geo_recommendation_rows(self, *, start: datetime, end: datetime) -> list[dict[str, Any]]:
        """Return GEO recommendation rows used by deterministic recommendations."""

        statement = (
            select(
                CrawlSession.website_id,
                GeoRecommendation.id,
                GeoRecommendation.priority,
                GeoRecommendation.severity,
                GeoRecommendation.title,
                GeoRecommendation.description,
                GeoRecommendation.created_at,
            )
            .join(GeoAnalysis, GeoAnalysis.id == GeoRecommendation.geo_analysis_id)
            .join(CrawlSession, CrawlSession.id == GeoAnalysis.crawl_session_id)
            .where(
                GeoRecommendation.priority <= 2,
                GeoRecommendation.created_at >= start,
                GeoRecommendation.created_at <= end,
            )
            .order_by(GeoRecommendation.priority.asc(), GeoRecommendation.created_at.desc())
            .limit(100)
        )
        return [
            {
    ...
    def active_alert_rows(self) -> list[dict[str, Any]]:
        """Return active or acknowledged alert rows for recommendations."""
        ...
        return [
            {
                "website_id": self._int_or_none(item.metadata_.get("website_id")) if item.metadata_ else None,
                "source_id": item.id,
